...
SELECT G.name, COUNT(B.booking_id) AS total_bookings FROM Guest G INNER JOIN Bookings B ON G.guest_id = B.guest_id
 GROUP BY G.guest_id, G.name HAVING COUNT(B.booking_id) > 1;
